...
(function () {
    ...
    var btcPrice = 0.00;
    var lastPrice = parseFloat($("#hilights > div:nth-child(1) > div.lastPrice > div.info").text()).toFixed(config.COIN_DECIMALS);
    var currentCoin;


    //set new price for btc
    function newBtcPrice(price) {
        
        var secondaryQuantity = parseFloat($('#secondaryBalance').text()).toFixed(config.COIN_DECIMALS);
        var primaryQuantity = parseFloat($('#primaryBalance').text());
        if(isNaN(lastPrice)){
            lastPrice = parseFloat($("#hilights > div:nth-child(1) > div.lastPrice > div.info").text()).toFixed(config.COIN_DECIMALS);         
        }
        var sellUsdValue = (secondaryQuantity * lastPrice) * price;
        var buyUsdValue = (primaryQuantity) * price;
        $("#sellUsdValue").text(sellUsdValue.toFixed(config.USD_DECIMALS));
        $("#buyUsdValue").text(buyUsdValue.toFixed(config.USD_DECIMALS));
    }

    //set new price for current coin
    var newPriceCurrentCoin = function (currentLastPrice) {
        lastPrice = currentLastPrice;
    }


    //when get ticker
    function onTicker(args) {
        if (args[0] == 'USDT_BTC') {
            newBtcPrice(parseFloat(args[1]));
        } else if (currentCoin && args[0] == currentCoin) {
            newPriceCurrentCoin(parseFloat(args[1]));
        }
    }

    //create DOM elements
    var init = function () {
        if (!$('#sellUsdValue').length) {
            $(".sellCol > div.head,.buyCol > div.head,.stop > div.head").css({ "height": "100px" });
            $(".sellCol > div.head > div:nth-child(2)").append("<div class=\"desc\">USD value</div>");
            $(".sellCol > div.head > div:nth-child(2)").append("<div class=\"desc\" id=\"sellUsdValue\"></div>");

            $(".buyCol > div.head > div:nth-child(2)").append("<div class=\"desc\">USD value</div>");
            $(".buyCol > div.head > div:nth-child(2)").append("<div class=\"desc\" id=\"buyUsdValue\"></div>");
        }
    }

    if (window.location.hash) {
        currentCoin = window.location.hash.replace('#', '').toUpperCase();
    }

    init();
    if (getPagePath().match(/exchange.*/)) {
